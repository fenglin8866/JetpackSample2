 ...
 *
 * @param状态 [Lifecycle.State]，其中“block”在新的协程中运行。如果生命周期低于该状态，则该协程将取消，如果再次处于该状态，则将重新启动。
 * @param块 当生命周期至少处于 [state] 状态时要运行的块。
 *
 * 在生命周期中重复执行
 */
 ...
 *         // The coroutine will be cancelled when the ON_STOP event happens and will
 *         // restart executing if the lifecycle receives the ON_START event again.
 *         //当生命周期至少为 STARTED 时，在协程中运行代码块。当ON_STOP事件发生时，协程将被取消，如果生命周期再次收到ON_START事件，协程将重新开始执行。
 *         lifecycleScope.launch {
